...
public class XxlJobTrigger {
    ...
    private static void processTrigger(XxlJobGroup group, XxlJobInfo jobInfo, int finalFailRetryCount, TriggerTypeEnum triggerType, int index, int total){
        ...
        Set<Integer> finishUpStreamJobList = new HashSet<Integer>();
        ...
        if (triggerType==TriggerTypeEnum.CRON){
            ...
            while (true){
                ...
                if (jobInfo.getUpStreamJobList()!=null && jobInfo.getUpStreamJobList().trim().length()>0) {
                    upStreamJobList = jobInfo.getUpStreamJobList().split(",");
                    List<XxlJobLog> xxlJobLogList = XxlJobAdminConfig.getAdminConfig().getXxlJobLogDao().pageLastList(jobInfo.getUpStreamJobList(),DateUtil.parseDateTodayStart(jobLog.getTriggerTime()), DateUtil.parseDateTodayEnd(jobLog.getTriggerTime()));
                    for (XxlJobLog xxlJobLog : xxlJobLogList) {
                        if (xxlJobLog.getHandleCode() == ReturnT.SUCCESS_CODE) {
                            finishUpStreamJobList.add(xxlJobLog.getJobId());
                        }
                    }
                }
        ...
        if (address != null && isContinueRun == true) {
        ...
        if(isContinueRun == false){
            triggerResult = new ReturnT<String>();
